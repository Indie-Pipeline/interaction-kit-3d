...
func _unhandled_input(event: InputEvent) -> void:
	if is_processing() and current_camera is Camera3D and event is InputEventMouseButton:
		...
		if interact_mouse_button == MOUSE_BUTTON_LEFT and PluginUtilities.is_mouse_left_click(event) \
...
func get_detected_interactable():
	...
	if PluginUtilities.is_mouse_visible() and result.has("collider") and result["collider"] is Interactable3D:
